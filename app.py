...
<div class="row"><label>종목풀 갱신(초)<input name="universe_refresh_seconds" type="number" min="120" max="900"></label><label>최저 주가($)<input name="min_stock_price" type="number" step="0.5" min="1" max="20"></label></div>
...
$('#settings').onsubmit=async e=>{e.preventDefault();let d=Object.fromEntries(new FormData(e.target));for(let k of ['max_positions','position_pct','cash_reserve_pct','stop_loss_pct','take_profit_pct','min_score','poll_seconds','universe_refresh_seconds','min_stock_price'])d[k]=Number(d[k]);try{render(await api('/api/config',d));toast('설정 저장 완료')}catch(err){toast(err.message)}};
...
class Engine:
    def __init__(self,store):
        ...
        self.stop_event=threading.Event(); self.thread=None; self.last_signal='대기'; self.last_candidates=[]; self.last_prices={}; self.dynamic_symbols=[]; self.dynamic_meta={}; self.dynamic_updated_at=0.0; self.dynamic_price_cap=0.0
    def start(self):
    ...
    def _discover_symbols(self,cash,equity,held_symbols,force=False):
        ...
        if isinstance(self.market,KISMarket) and price_cap>=self.config.min_stock_price:
            try:
                ...
                self.store.set('last_discovery_error',str(e))
    ...
    def scan(self,execute=False):
        ...
        positions=self._snapshot_positions(force=True); held_symbols={p['symbol'] for p in positions}
        try:
            cash=self._reference_cash(force=True)
        except Exception as e:
        ...
        if not execute:
            ...
            self.last_signal=f'실시간 종목풀 {pool_count}개 · 매수조건 통과 {ready}개' + (f' · 오류 {len(errors)}개' if errors else '')
        ...
        self.last_signal=(f'{bought}종목 매수 주문' if bought else f'조건 대기 · 후보 {", ".join(top_symbols[:3]) or "없음"}') + (f' · 오류 {len(errors)}' if errors else '')
        self.store.set('last_scan_errors',errors[-5:] if errors else [])
        return self.state()
