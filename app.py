...
DEFAULT_UNIVERSE = (
    ...
    "NASD:AMD,NASD:TSLA,NASD:QQQ,NASD:SOFI,NASD:INTC,NASD:CSCO,NASD:PYPL,"
    "NYSE:BAC,NYSE:F,NYSE:PFE,NYSE:T,NYSE:VZ,NYSE:GM,NYSE:KO"
)
...
const cs=s.candidates||[];$('#candidates').innerHTML=cs.length?cs.map((c,i)=>`<div class="item"><div><b>${i+1}. ${c.symbol}</b><div class="muted">${money(c.price)} · 추세 ${c.trend_pct}% · 모멘텀 ${c.momentum_pct}%${c.eligible?(c.affordable?' · 매수가능':' · 자금부족'):''}</div></div><div class="score ${(c.eligible&&c.affordable)?'good':''}">${c.score.toFixed(2)}</div></div>`).join(''):'<p class="muted">아직 분석 전입니다.</p>';
...
$('#settings').onsubmit=async e=>{e.preventDefault();let d=Object.fromEntries(new FormData(e.target));for(let k of ['max_positions','position_pct','cash_reserve_pct','stop_loss_pct','take_profit_pct','min_score','poll_seconds'])d[k]=Number(d[k]);try{render(await api('/api/config',d));toast('설정 저장 완료')}catch(err){toast(err.message)}};
...
class Engine:
    ...
    def scan(self,execute=False):
        ...
        positions=self._snapshot_positions(force=True); held={p['symbol']:p for p in positions}
        equity,cash=self._equity_and_cash(positions,candidates,force_cash=True)
        ...
        self.last_candidates=sorted(candidates,key=lambda c:(1 if c['eligible'] and c['affordable'] else 0,1 if c['eligible'] else 0,c['score']),reverse=True)[:8]
        if not execute:
            ready=sum(1 for c in candidates if c['eligible'] and c['affordable'])
            self.last_signal=f'분석 완료 · 매수가능 후보 {ready}개' + (f' · 오류 {len(errors)}개' if errors else '')
            if errors: self.store.set('last_scan_errors',errors[-5:])
            return self.state()
        ...
        if slots>0:
            ...
            reserve_cash=max(0.0,cash*self.config.cash_reserve_pct/100)
            available_cash=max(0.0,cash-reserve_cash)
            ...
            for c in candidates:
                ...
                if one_share>available_cash or one_share>per_cap:
                    continue
                remaining_slots=max(1,slots-bought)
                split_budget=available_cash/remaining_slots
                budget=min(available_cash,per_cap,max(split_budget,one_share))
                try:
                    if self.broker.buy(c['symbol'],price,budget,f'소액계좌 자동선정 점수 {c["score"]:.2f}'):
                        bought+=1; held.add(c['symbol']); available_cash=max(0.0,available_cash-budget)
    ...
    def state(self):
        ...
        return {'running':self.running,'mode':'KIS 실계좌 · 소액계좌 자동분산' if self.live else '테스트 · 소액계좌 자동분산','config':asdict(self.config),'cash':round(cash,2),'positions':positions,'candidates':self.last_candidates,'equity':round(equity,2),'profit':round(profit,2),'profit_pct':round(profit_pct,2),'day_profit_pct':round(day_pct,2),'last_signal':self.last_signal,'scan_errors':self.store.get('last_scan_errors',[]) or [],'trades':self.store.trades(40)}
