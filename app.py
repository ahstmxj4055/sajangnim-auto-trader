...
<label>운용 모드<input value="수익우선 · 강한 신호만 · 투자금 자동결정" disabled></label>
<div class="row"><label>최대 보유종목<input name="max_positions" type="number" min="1" max="3"></label><label>투자금 결정<input value="자동 · 점수/추세/변동성/당일손익 반영" disabled></label></div>
...
<p class="notice">⚠️ 수익우선 모드는 강한 신호만 선별하고 투자금도 자동 조절하지만 수익을 보장하지 않습니다. 손절·일손실 제한은 계좌 보호를 위해 유지됩니다.</p>
...
const cs=s.candidates||[];$('#candidates').innerHTML=cs.length?cs.map((c,i)=>`<div class="item"><div><b>${i+1}. ${c.symbol}</b><div class="muted">${money(c.price)} <span class="mini-krw">${krw(c.price,fx)}</span> · 추세 ${c.trend_pct}% · 모멘텀 ${c.momentum_pct}% · 변동 ${c.volatility}%${c.auto_position_pct?` · 자동비중 ${c.auto_position_pct}% · 예산 ${money(c.auto_budget)}`:''}${c.eligible?' · 매수조건 통과':' · 대기'}</div></div><div class="score ${c.eligible?'good':''}">${c.score.toFixed(2)}</div></div>`).join(''):'<p class="muted">현재 자금으로 살 수 있는 후보를 찾는 중입니다.</p>';
...
class Config:
    ...
    max_positions: int = 2
    position_pct: float = 30.0  # legacy hard cap 호환용
    ...
    min_trend_pct: float = 0.10
    min_momentum_pct: float = 0.20
    max_volatility_pct: float = 2.50
    auto_min_position_pct: float = 6.0
    auto_max_position_pct: float = 30.0
    loss_throttle_start_pct: float = 0.50
    strategy_version: int = 4
    poll_seconds: int = 30
...
class Engine:
    def __init__(self,store):
        ...
        # 수익우선 v4: 기존 저장설정도 한 번 자동 마이그레이션한다.
        if int(saved.get('strategy_version',0) or 0) < 4:
            self.config.strategy_version=4
            self.config.max_positions=2
            self.config.position_pct=30.0
            self.config.cash_reserve_pct=20.0
            self.config.min_score=0.30
            self.config.min_trend_pct=0.10
            self.config.min_momentum_pct=0.20
            self.config.max_volatility_pct=2.50
            self.config.auto_min_position_pct=6.0
            self.config.auto_max_position_pct=30.0
            self.config.loss_throttle_start_pct=0.50
            self.config.min_hold_minutes=20
            self.config.rebuy_cooldown_minutes=60
            self.config.daily_loss_limit_pct=1.5
            self.store.set('config',asdict(self.config))
    ...
    def _discover_symbols(self,cash,equity,held_symbols,force=False):
        ...
        per_cap=max(0.0,equity*self.config.auto_max_position_pct/100)
    ...
    def _score(self,symbol):
        ...
        eligible=(last>long and trend>=self.config.min_trend_pct and mom>=self.config.min_momentum_pct and vol<=self.config.max_volatility_pct and score>=self.config.min_score)
    ...
    def _auto_position_pct(self,candidate,day_pct=0.0):
        # 점수·추세·모멘텀은 비중을 키우고, 변동성과 당일 손실은 비중을 줄인다.
        score=float(candidate.get('score',0) or 0); trend=max(0.0,float(candidate.get('trend_pct',0) or 0)); mom=max(0.0,float(candidate.get('momentum_pct',0) or 0)); vol=max(0.0,float(candidate.get('volatility',0) or 0))
        score_strength=max(0.0,min(1.0,(score-self.config.min_score)/0.80))
        trend_strength=max(0.0,min(1.0,trend/1.00))
        momentum_strength=max(0.0,min(1.0,mom/1.50))
        volatility_penalty=max(0.0,min(0.70,vol/max(0.5,self.config.max_volatility_pct)))
        conviction=0.58*score_strength+0.20*trend_strength+0.22*momentum_strength-0.32*volatility_penalty
        conviction=max(0.0,min(1.0,conviction))
        pct=self.config.auto_min_position_pct+(self.config.auto_max_position_pct-self.config.auto_min_position_pct)*conviction
        # 당일 손실이 시작되면 복구매매 대신 자동으로 투자금을 줄인다.
        if day_pct<0:
            throttle_start=max(0.01,self.config.loss_throttle_start_pct)
            if abs(day_pct)>=throttle_start:
                loss_ratio=min(1.0,abs(day_pct)/max(throttle_start,self.config.daily_loss_limit_pct))
                pct*=max(0.45,1.0-0.55*loss_ratio)
        return round(max(self.config.auto_min_position_pct,min(self.config.auto_max_position_pct,pct)),2)
    ...
    def scan(self,execute=False):
        ...
        max_single_budget=min(max(0.0,equity*self.config.auto_max_position_pct/100),max(0.0,cash-reserve_cash))
        preview_base=float(self.store.get('day_start_equity',equity) or equity); preview_day_pct=(equity/preview_base-1)*100 if preview_base>0 else 0
        for c in candidates:
            auto_pct=self._auto_position_pct(c,preview_day_pct)
            auto_budget=min(max(0.0,equity*auto_pct/100),max(0.0,cash-reserve_cash),max_single_budget)
            c['auto_position_pct']=auto_pct; c['auto_budget']=round(auto_budget,2)
            c['affordable']=bool(c['price']*1.002<=auto_budget)
        # 화면에는 프로그램이 계산한 자동예산으로 실제 1주 이상 살 수 있는 종목만 노출한다.
        ...
        if slots>0:
            ...
            live_equity=max(0.0,cash+sum(float(p['qty'])*float(p['price']) for p in positions))
            for c in affordable:
                ...
                auto_pct=self._auto_position_pct(c,day_pct)
                budget=min(available_cash,max(0.0,live_equity*auto_pct/100),max(0.0,live_equity*self.config.auto_max_position_pct/100))
                if one_share>budget: continue
                try:
                    self._log(f'[BUY] {c["symbol"]} 자동비중 {auto_pct:.1f}% · 예산 ${budget:.2f} · 점수 {c["score"]:.2f} 주문 시도','warn')
                    if self.broker.buy(c['symbol'],price,budget,f'수익우선 자동선정 · 점수 {c["score"]:.2f} · 자동비중 {auto_pct:.1f}%'):
    ...
    def update(self,raw):
        ...
        if not (0<=c.min_trend_pct<=5): raise ValueError('최소 추세는 0~5%만 가능합니다')
        if not (0<=c.min_momentum_pct<=10): raise ValueError('최소 모멘텀은 0~10%만 가능합니다')
        if not (.2<=c.max_volatility_pct<=10): raise ValueError('최대 변동성은 0.2~10%만 가능합니다')
        if not (1<=c.auto_min_position_pct<=c.auto_max_position_pct<=40): raise ValueError('자동비중은 최소 1%, 최대 40% 범위여야 합니다')
        if not (.1<=c.loss_throttle_start_pct<=5): raise ValueError('손실축소 시작은 0.1~5%만 가능합니다')
    ...
    def state(self):
        ...
        return {'running':self.running,'mode':'KIS 실계좌 · 수익우선 자동비중' if self.live else '테스트 · 수익우선 자동비중','config':asdict(self.config),'cash':round(cash,2),'positions':positions,'candidates':self.last_candidates,'equity':round(equity,2),'principal':round(principal,2),'profit':round(profit,2),'profit_pct':round(profit_pct,2),'day_profit_pct':round(day_pct,2),'fx_rate':USD_KRW_RATE,'last_signal':self.last_signal,'dynamic_pool':self.dynamic_symbols,'dynamic_price_cap':round(self.dynamic_price_cap,2),'pool_age_seconds':round(max(0,time.time()-self.dynamic_updated_at)) if self.dynamic_updated_at else None,'scan_errors':self.store.get('last_scan_errors',[]) or [],'activity':list(self.activity),'trades':self.store.trades(40)}
